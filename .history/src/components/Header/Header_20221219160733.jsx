    ...
    <Box sx={{ flexGrow: 1 }}>
      <AppBar position="static" sx={{ backgroundColor: 'SandyBrown' }}>
        <Toolbar>
          ...
          {isLoggedIn && (
            <>
              <Typography className="user-login" variant="h6" component="div" sx={{ flexGrow: 0.01, color: 'Linen' }}>
                {login}
              </Typography>
              <Button className="login-button" variant="text" sx={{ color: 'white' }} onClick={logOut}>Log Out</Button>
            </>
          )}
        </Toolbar>
      </AppBar>
