    ...
    <Box sx={{ flexGrow: 1 }}>
      <AppBar position="static">
        <Toolbar>
          ...
          {!isLoggedIn && (
            <>
              <button className="login-button" type="button" onClick={() => openModal('login')}>{LOGIN_BUTTON_TEXT}</button>
              <button type="button" onClick={() => openModal('signUp')}>{SIGNUP_BUTTON_TEXT}</button>
            </>
          )}
          {isLoggedIn && (
            <>
              <p className="user-login">
                <Link className="login-link" to={`/users/${id}`}>{login}</Link>
              </p>
